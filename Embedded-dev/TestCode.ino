...
void loop()
{
  ...
  float adjusted = millis()/1000/secondsperphase - x;
  if (adjusted > 1 && x < 26){
    ...
    particleSensor.setup(ledBrightness, 2^(x-26), ledMode, sampleRate, pulseWidth, adcRange);
    x++;
  }
  else if (x >= 32 && x < 40){
    particleSensor.setup(ledBrightness, sampleAverage, ledMode, 50*2^(x-32), pulseWidth, adcRange);
    x++;
  }
  else if (x >= 40 && x < 44){
    switch (x-40){
